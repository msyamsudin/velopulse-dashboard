...
class FakeService {
  constructor(private characteristics: Record<string, FakeCharacteristic>) {}

  async getCharacteristic(name: string) {
    const c = this.characteristics[name];
    if (!c) throw new Error(`No characteristic ${name}`);
    return c;
  }
}
...
class FakeServer {
  ...
  constructor(private services: Record<string, FakeService>) {}

  async connect() {
    this.connectCalls += 1;
    if (this.failConnect) throw new Error('radio busy');
    return this;
  }

  async getPrimaryService(name: string) {
...
class FakeDevice {
  ...
  get gatt() {
    return {
      connect: () => this.server.connect(),
      disconnect: () => {
        queueMicrotask(() => this.emit('gattserverdisconnected'));
      },
    };
  }
...
const makeHrDevice = () => {
  ...
  const service = new FakeService({ heart_rate_measurement: characteristic });
  const server = new FakeServer({ heart_rate: service });
  const raw = new FakeDevice(server);
  const device = raw as unknown as BluetoothDevice;
  return { device, raw, characteristic, server };
};

const makeBikeDevice = () => {
  const characteristic = new FakeCharacteristic();
  const service = new FakeService({ indoor_bike_data: characteristic });
  const server = new FakeServer({ fitness_machine: service });
  const raw = new FakeDevice(server);
  const device = raw as unknown as BluetoothDevice;
  return { device, raw, characteristic, server };
};
...
describe('auto-reconnect', () => {
  beforeEach(() => {
    // Exclude queueMicrotask from the faked set: vitest fakes it by default,
    // which breaks advanceTimersByTimeAsync for timers whose callbacks await
    // promises (the reconnect loop's establish() chain) — timers never fire.
    vi.useFakeTimers({
  ...
  it('reconnects after a disconnect and resumes the connected state', async () => {
    const { device, raw, server } = makeHrDevice();
    await attachHrDevice(device, useBluetoothStore.setState, useBluetoothStore.getState);
    expect(useBluetoothStore.getState().hrConnected).toBe(true);
    expect(server.connectCalls).toBe(1);

    await raw.emit('gattserverdisconnected');
    expect(useBluetoothStore.getState().hrConnected).toBe(false);

    await vi.advanceTimersByTimeAsync(RECONNECT_INITIAL_DELAY_MS + 50);
    expect(server.connectCalls).toBe(2);
    expect(useBluetoothStore.getState().hrConnected).toBe(true);
  });

  it('runs only ONE reconnect loop when disconnect events arrive in a burst', async () => {
    const { device, raw, server } = makeHrDevice();
    await attachHrDevice(device, useBluetoothStore.setState, useBluetoothStore.getState);

    await raw.emit('gattserverdisconnected');
    await raw.emit('gattserverdisconnected'); // duplicate while a loop is pending
    await vi.advanceTimersByTimeAsync(RECONNECT_INITIAL_DELAY_MS + 50);

    // initial connect + exactly one reconnect, not two racing loops
    expect(server.connectCalls).toBe(2);
    expect(useBluetoothStore.getState().hrConnected).toBe(true);
  });

  it('registers only one disconnect handler when the device is attached twice (dev StrictMode double-mount)', async () => {
    const { device, raw, server } = makeHrDevice();
    await attachHrDevice(device, useBluetoothStore.setState, useBluetoothStore.getState);
    await attachHrDevice(device, useBluetoothStore.setState, useBluetoothStore.getState);
    expect(raw.listeners['gattserverdisconnected'].length).toBe(1);

    await raw.emit('gattserverdisconnected');
    await vi.advanceTimersByTimeAsync(RECONNECT_INITIAL_DELAY_MS + 50);
    // two attaches + exactly ONE reconnect loop (a duplicate listener would give 4)
    expect(server.connectCalls).toBe(3);
    expect(useBluetoothStore.getState().hrConnected).toBe(true);
  });

  it('keeps retrying with backoff instead of giving up after 5 attempts', async () => {
    const { device, raw, server } = makeHrDevice();
    await attachHrDevice(device, useBluetoothStore.setState, useBluetoothStore.getState);
    expect(server.connectCalls).toBe(1);
    ...
    // Attempts land at ~300/900/2100/4500/9300/14100/18900 ms — well past the
    // old 5-attempt cap — and the loop must still be alive.
    await vi.advanceTimersByTimeAsync(20_000);
    expect(useBluetoothStore.getState().hrConnected).toBe(false);
    expect(server.connectCalls).toBeGreaterThanOrEqual(8);

    server.failConnect = false;
    await vi.advanceTimersByTimeAsync(10_100); // next scheduled attempt succeeds
    expect(useBluetoothStore.getState().hrConnected).toBe(true);
  });

  it('aborts auto-reconnect on manual disconnect', async () => {
    const { device, server } = makeHrDevice();
    await attachHrDevice(device, useBluetoothStore.setState, useBluetoothStore.getState);

    useBluetoothStore.getState().disconnect();
    await vi.advanceTimersByTimeAsync(30_000);

    expect(server.connectCalls).toBe(1); // no reconnect attempts at all
    expect(useBluetoothStore.getState().hrConnected).toBe(false);
    expect(useBluetoothStore.getState().hrDevice).toBeNull();
  });

  it('fails the initial connect instead of silently succeeding when a service is missing', async () => {
    const service = new FakeService({}); // no heart_rate_measurement characteristic
    const server = new FakeServer({ heart_rate: service });
    const device = new FakeDevice(server) as unknown as BluetoothDevice;

    await expect(
      attachHrDevice(device, useBluetoothStore.setState, useBluetoothStore.getState)
    ).rejects.toThrow();
    expect(useBluetoothStore.getState().hrConnected).toBe(false);
    expect(useBluetoothStore.getState().hrDevice).toBeNull();
  });

  it('reconnects the bike after a disconnect', async () => {
    const { device, raw, server } = makeBikeDevice();
    await attachBikeDevice(device, useBluetoothStore.setState, useBluetoothStore.getState);
    expect(useBluetoothStore.getState().bikeConnected).toBe(true);
