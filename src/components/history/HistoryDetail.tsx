...
export const HistoryDetail = ({
  ...
  const achievements = useMemo(() => {
    return detectSessionAchievements(
      session,
      allSessions && allSessions.length > 0 ? allSessions : [session],
      t
    );
        ...
        <div className="max-w-7xl mx-auto w-full flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-3">
                ...
                {achievements.sessionIndex > 0 && (
                  <span className="rounded border border-hw-accent/30 bg-hw-accent/10 px-2.5 py-1 text-[9px] font-mono uppercase tracking-widest text-hw-accent font-bold">
                    {t('Ride #{index}', { index: achievements.sessionIndex })}
                  </span>
                )}
              ...
              <div className="flex items-center gap-3 text-hw-muted text-xs font-mono uppercase tracking-widest mt-1">
                <span className="flex items-center gap-1"><Calendar size={12} /> {formatDate(session.date, locale)}</span>
          ...
          <div className="flex flex-wrap gap-3">
            <button
              ...
              title={t('Generate visual card for Instagram, X, or WhatsApp')}
            >
              <Share2 size={12} />
              {t('Share Card')}
            </button>
            <button
              ...
              title={t('Export for Strava/Garmin manual upload')}
            >
              <Download size={12} />
              {t('Export TCX')}
            </button>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-mono uppercase tracking-[0.2em] font-bold text-amber-300 flex items-center gap-2">
                      <Sparkles size={14} />
                      {t(achievements.isCenturion ? 'Centurion Milestone Reached!' : 'Workout Achievements Unlocked!')}
                    </div>
                    <div className="text-sm font-bold text-white mt-0.5">
                      {t(achievements.isCenturion
        ...
        <div className="flex flex-col gap-6">
          {autoInsights.length > 0 && (
            <div className="hardware-card border-hw-muted/20 p-4">
              <div className="mb-4 border-b border-white/5 pb-3">
                ...
                <div className="text-[11px] font-mono uppercase tracking-[0.12em] text-white/45 mt-1">{t('Session interpretation from current telemetry')}</div>
              </div>
          ...
          <div className="hardware-card border-hw-muted/20 p-4">
            <div className="mb-4 flex items-center justify-between gap-3 border-b border-white/5 pb-3">
              <div>
                <div className="text-[9px] font-mono uppercase tracking-[0.2em] text-hw-muted">{t('Performance')}</div>
                <div className="text-[11px] font-mono uppercase tracking-[0.12em] text-white/45">{t('Secondary metrics and session load')}</div>
              </div>
              <div className="text-[10px] font-mono uppercase tracking-[0.12em] text-hw-accent">{zoneInsight}</div>
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <MiniMetric label={t('Avg HR')} value={session.stats.avgHr} unit="bpm" icon={<Heart size={11} />} colorClass="text-red-500" />
              <MiniMetric label={t('Cadence')} value={session.stats.avgCadence} unit="rpm" icon={<Bike size={11} />} colorClass="text-hw-accent" />
          ...
          {hrrScore !== null && (
            <div className="hardware-card border-emerald-400/20 bg-emerald-400/5 p-4">
              <div className="mb-4 flex items-center justify-between gap-3 border-b border-emerald-400/10 pb-3">
                <div>
                  <div className="text-[9px] font-mono uppercase tracking-[0.2em] text-emerald-300">{t('Heart Rate Recovery')}</div>
                  <div className="text-[11px] font-mono uppercase tracking-[0.12em] text-white/45 mt-1">{t('2 minute post-ride recovery score')}</div>
                </div>
                <Heart size={16} className="text-emerald-300" fill="currentColor" />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <MiniMetric label={t('HRR Score')} value={hrrScore} unit="bpm" icon={<Heart size={11} />} colorClass="text-emerald-300" />
          ...
          <div className="hardware-card border-hw-muted/20 p-4">
            <div className="mb-4 flex items-center justify-between gap-3 border-b border-white/5 pb-3">
              <div className="text-[10px] text-hw-muted uppercase font-mono flex items-center gap-2">
                <Activity size={10} className="text-hw-accent" /> {t('Heart Rate Zones')}
              </div>
              <button
              ...
              >
                {t(showZoneBpm ? 'Hide BPM' : 'Show BPM')}
              </button>
            </div>
            ...
            {isDetailReady ? (
              ...
              <div className="h-[420px] w-full bg-black/20 rounded-xl flex items-center justify-center border border-white/5">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="text-[10px] font-mono text-hw-muted uppercase tracking-widest">{t('Generating Telemetry...')}</span>
                </div>
              </div>
            )}
          </div>
